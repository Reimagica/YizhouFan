...
export type AnswerTopic = "profile" | "publications" | "talks" | "people";
...
export function selectPublicSources(question: string, lang: Language, topics: AnswerTopic[] = []): AnswerLink[] {
  ...
  if (topics.includes("profile") || /teach|course|award|honou?r|project|position|bio|contact|email|research|研究|课程|教学|荣誉|奖励|项目|任职|简介|联系|邮箱/u.test(query)) add(0);
  if (topics.includes("publications") || /publication|paper|article|book|wrote|written|authored|论文|成果|著作|文章|期刊|会议/u.test(query)) add(1);
  if (topics.includes("talks") || /talk|keynote|lecture|presentation|报告|演讲|讲座|课件/u.test(query)) add(2);
  if (topics.includes("people") || /student|member|team|lab|课题组|团队|成员|学生|博士后/u.test(query)) add(3);
...
function publicationUrl(publication: PublicPublication) {
  ...
  return `https://doi.org/${publication.doi.replace(/^https?:\/\/(?:dx\.)?doi\.org\//iu, "")}`;
}
