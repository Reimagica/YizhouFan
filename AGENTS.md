...
- 每次请求均为独立单轮，不向模型发送浏览器内的历史问答；公开知识上下文最多 28,000 字符。Profile、Courses、Talks、People 及公开履历类区块使用独立预算并始终置于成果之前；成果按访客问题对题名、译名、关键词、摘要、作者、载体和年份加权检索，最多注入 10 条相关已发布成果，只有这些候选携带摘要与关键词。模型输出上限 1,100 tokens、18 秒超时、低温度生成。访客问题和 CMS 已发布文本均按数据而非指令处理。
...
### 搜索引擎可见性与页面语义（2026-09-17）

- 每个可索引栏目只承载一个明确主题，使用唯一、准确的双语 Title 与 Description；页面保持单一 H1 和正常的 H2/H3 层级，不为搜索引擎新增隐藏文字或关键词堆砌。
- `/en/*` 与 `/zh/*` 均设置 canonical、双向 `hreflang` 和 `x-default`（指向英文版本）；当前 canonical 主机继续使用 `https://yizhoufan.com`，apex/`www` 信号冲突按用户要求另行处理，不在本轮改动。
- 首页输出基于已确认公开资料的 `ProfilePage` + `Person` JSON-LD，只允许姓名、公开头像、职务、机构、研究方向、Google Scholar、ORCID 与北大官方教师页等公开事实；不得把私人简历或未确认项目写入结构化数据。
- `/[lang]/ask` 保持正常可访问和站内可发现，但标记为 `noindex, follow`，且不写入 sitemap；`/api/*` 在 robots.txt 中禁止抓取。sitemap 只列出真实存在且希望收录的 HTTPS 双语页面，不使用每次构建生成的伪 `lastModified`。
- 公开正文图片在 Studio 新增时必须填写准确的中英文 alt，前台优先当前语言并回退另一语言；人物头像继续使用真实姓名作为 alt。装饰性内容不得用关键词伪造替代文本。
- 现有导航和正文链接必须使用普通可抓取链接，不新增隐藏链接；SEO 调整不得明显改变当前页面布局与视觉表现。
...
- `yizhoufan.com` 当前可访问；apex 与 `www` 的权威域名信号仍有冲突并按用户要求暂缓处理。代码中的 canonical 与 sitemap 主机暂维持 `https://yizhoufan.com`，不得在未确认最终主域前自行切换。
...
└── server/
    ...
    ├── knowledge-retrieval.ts    ← AI 核心区块预算、成果相关性检索与知识文本硬上限
...
- `app/globals.css`：移除旧 `people-tabs` 相关样式；新增 `.people-grid`（桌面 3 列 `repeat(3, minmax(0,1fr))`、`≤980px` 2 列、`≤560px` 1 列，`max-width: 1100px; margin-inline: auto`）与 `.person-card` 系列——边框 `1px solid var(--line)`、圆角 16px、背景 `--surface`、阴影 `0 12px 34px rgba(42,34,28,0.04)`（与课程/成果卡一致）；`.person-card__portrait` 当前统一为 `aspect-ratio: 1/1` 的正方形，适配成员资料征集所用的 1:1 大头照，无图时为渐变背景 + 首字母缩写；`.person-card__links a:focus-visible` 保留下划线，键盘焦点样式未移除。
...
### 2026-08-25 - People 头像比例与资料征集口径

- 团队成员卡片头像由 4:5 调整为 1:1 正方形；继续使用 Next Image `fill`、响应式 `sizes` 与 `object-fit: cover`，并由正方形容器预留稳定空间，避免布局跳动。
- 成员简介征集进一步收敛：中文建议 50–80 字、最多 100 字；英文建议 30–50 词、最多 60 词。内容以 1–2 句概括当前身份、研究兴趣或主要方向，中英文事实必须一致并经本人确认。
- 头像征集建议使用 1:1、至少 800×800 像素的清晰正面照；JPG/JPEG、PNG 优先，WebP 也可正常使用；“不超过 5 MB”是便于收集、传输和页面性能的建议标准，不得表述为当前 Sanity `person.portrait` 的系统硬限制。当前 Person Schema 未配置头像 MIME 或 5 MB 校验，若未来需要后台强制执行，须另行增加校验并重新部署 Studio。

### 2026-08-28 - People 首批4位成员资料上线

- 完整读取用户提供的《朱桃林简介》《夏梦雨简介(2)》《个人信息-许家奇(1)》三份 DOCX；Pandoc 与 DOCX 内嵌媒体结构一致确认每份均含中英文简介及一张 JPEG 头像。标准 DOCX PNG 渲染因本机缺少 LibreOffice/pdf2image 未完成，不以此声称版式验收；本轮只使用经提取并人工核对的文字与图片内容。
- 写入前全量导出 Sanity Production（119份导出文档、90个既有资产）至仓库外 `E:/科研/课题组网站/YizhouFan-private/Sanity备份/yizhoufan-production-before-people-20260828.tar.gz`，避免私人资料与备份进入 Git。
- 复用现有 `person-001`（许家奇）、`person-002`（夏梦雨）、`person-006`（朱桃林），未创建重复 Person。按提交材料写入入学/入组年份、原文双语简介与3张头像；朱桃林英文姓名按最新材料由 `Taolin Zhu` 更新为 `Zhu Taolin`。用户随后明确确认夏梦雨英文显示名使用 `Marine`，已将 `person-002.name.en` 从 `Mengyu Xia` 更新为 `Marine`。
- 写入后 Sanity 只读复核：9位 Person 全部 published，3位同时具备 `enrollmentYear`、中英文 `bio` 与 `portrait`，其余6位没有简介/头像并继续显示克制占位；Asset 总数由90增至93（89 file、4 image）。未修改其余6位成员的姓名、身份、排序或发布状态。
- 生产 People 页面通过 Sanity webhook/缓存刷新自动读取内容；本机访问 Vercel 时因 `yizhoufan.com` 尚无 DNS 且当前网络无法连接 `vercel.app`，未完成浏览器端页面验收。1:1头像 CSS 改动已获用户授权，随本轮 `main` 推送触发 Vercel Production 部署。
...
- 最终验证：4个 Sanity 成员图片 URL 均返回 HTTP 200；本地生产构建的中英文 People 页面包含4位已补全成员与5位占位；ESLint 0 error/0 warning、TypeScript 通过、Next.js Production build 通过、38/38 测试通过、`git diff --check` 干净。
...
### 2026-09-08 - AI 称谓与课题组事实约束

- 根据线上问答截图排查确认，错误称谓来自三处本地 Harness：系统提示将助手绑定到 `Dr. Yizhou Fan and FanLearn Lab`，公开知识块把课题组写成有正式名称的 `FanLearn Lab`，英文示例问题与静态回退简介也带有 `Dr.`/“博士”。截图中的模型回答只作为问题证据，不作为公开事实来源。
- 系统提示改为服务范逸洲个人学术网站，并明确：英文只称 `Yizhou Fan`，中文只称“范逸洲”或“范逸洲老师”；职称只可逐字采用 `PUBLIC PROFILE Position` 中已有信息；不得为课题组创造专名或无依据的领导关系，统一使用 `the research group`/`the team` 或“课题组”/“研究团队”。
- 公开知识块移除 `FanLearn Lab` 及“led by”断言，改为明确没有已公开确认的课题组专名，研究领域只引用 Profile 已发布的 `researchInterests`；静态回退简介和前台英文示例同步移除 `Dr.` 与旧专名。
- 输出守卫增加确定性称谓清洗：即使模型违背提示，仍会将旧课题组名称及 `Dr. Fan`、`Dr. Yizhou Fan`、“范逸洲博士”、“范博士”等替换为中性、已确认称谓；新增中英文回归测试覆盖该兜底。

### 2026-09-08 - AI 公开知识按问题检索与区块预算

- AI 公开知识由“固定拼接全部 92 条成果后截断”改为“固定核心资料 + 按问题检索成果”：Profile、Courses、Talks、People、荣誉、公开项目与课题组口径先按独立区块预算写入，成果区固定置于最后，整体继续执行 28,000 字符硬上限。
- Profile 新纳入公开 Bio、任职经历与学术服务；课程新纳入授课角色和开设时间；报告新纳入当前语言简介；成员继续纳入双语姓名、身份、后台年份与公开简介。公开项目仍只读取 `publiclyConfirmed == true` 的白名单内容。
- 新增纯函数成果检索：对访客问题与成果的原始/翻译题名、关键词、双语摘要、作者、载体和年份加权匹配，精确题名优先；每次最多向模型提供 10 条相关成果，并为入选成果附摘要与关键词。非成果问题不再携带无关成果；无具体匹配的泛成果问题按代表性、年份和题名稳定选择。
- 全量已发布成果仍保留在服务端内存中，只用于核验模型返回的 `publicationIds`、补齐完整题名和选择真实来源链接；模型提示明确所见成果只是问题相关子集，不得将其误称为完整成果列表或断言未入选成果不存在。
- 新增 `tests/public-knowledge.test.mjs`，覆盖中英文相关性选择、非成果问题排除、Profile/Course/Talk/People 字段进入知识、摘要与关键词进入相关成果、最多 10 条以及极端长文本下核心区块和 28,000 字符上限。遵循 Next.js 服务端性能约束，五类独立 Sanity 读取继续通过 `Promise.all` 并行执行。

### 2026-09-08 - People 按入学年份与成员身份排序

- Person Schema 新增结构化 `memberRole`，后台以单选方式提供“博士后 / 博士生 / 硕转博 / 硕士生 / 已毕业 / 其他”六个值；新增或编辑成员必须同时填写既有必填 `enrollmentYear` 与 `memberRole`。两个字段只用于排序、不在前台展示；`position` 保留为前台双语身份文案，与排序身份解耦。
- 前台排序改为：有入学年份者在前，年份从近到远；同年依次为博士后、博士生、硕转博、硕士生、已毕业、其他；同身份再按当前语言姓名稳定排序。旧 `order` 字段不再参与排序，在 Studio 中隐藏但不删除历史数据。
- 为兼容尚未回填 `memberRole` 的 Production 文档，排序纯函数会从已发布的中英文身份文案确定性识别六类；先识别博士后，并将“毕业生 · 香港大学博士研究生”优先识别为已毕业，避免被其中的博士字样误排；无法识别的旧身份归入“其他”。
- Sanity 查询与 `PublicPerson` 类型已纳入 `memberRole`；静态回退 9 人补入已确认年份及身份，确保 Sanity 不可用时顺序一致。新增测试覆盖年份优先级、六类身份顺序、旧 `order` 失效、历史文案兼容、双语姓名 tie-break、前台不展示排序身份与输入数组不变。
- 第一版四类排序曾随提交 `d54ebf0` 推送并部署；用户随即补充博士后与其他两类，本节以最终六类规则覆盖此前四类口径。写入前完整导出 Production（120份文档、110个资产）至仓库外 `E:/科研/课题组网站/YizhouFan-private/Sanity备份/yizhoufan-production-before-member-roles-20260908.tar.gz`；随后只为9位已发布 Person 回填 `memberRole`：1位博士后、3位博士生、4位硕士生、1位已毕业，姓名、年份、展示身份、简介、头像与发布状态均未修改。
- 最终验证：ESLint 0 error/0 warning；Next.js 16.2.12 Production build通过并生成24个页面；Sanity Studio build通过；全量测试54/54通过。最终六类 Schema 已部署至 `https://yizhoufan.sanity.studio/`，新增或编辑成员时 `enrollmentYear` 与 `memberRole` 均为必填，前台不展示二者。

### 2026-09-08 - 肖琳霏与马玲头像由用户在 Studio 手动替换
...
### 2026-09-08 - Person Studio 填写体验与未知字段清理（已部署，后续交互调整待部署）

- Person Schema 的“身份/状态”最初改为模板/其他双模式：模板模式根据“入学年份 + 成员身份”自动生成“北京大学教育学院XXXX级XXX”及预设英文，选择“其他”后显示中英文手写框；该版本的 `positionMode` 仅作为隐藏兼容字段保留，`position` 使用 `PersonPositionInput` 自定义输入。
...
### 2026-09-08 - 修复身份模板在只读文档上的回填异常（本地待部署）

- 用户反馈打开团队成员时出现 `Attempted to patch a read-only document`。原因是 `PersonPositionInput` 在 Studio 仅查看已发布文档时仍通过 `useEffect` 自动发出 `onChange` 补丁。
