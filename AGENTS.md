...
- 团队成员概览：已取消博士后、在读学生、毕业生三类前台分组，所有成员在同一页面按入学年份降序（近→远）排列；同一年份固定按博士生、硕转博、硕士生、已毕业、其他既有身份排序，再按当前语言显示名稳定排序。Studio 新增成员时必须填写 `enrollmentYear` 并从硕士生、博士生、硕转博、已毕业中选择 `memberRole`，两个字段均不在前台卡片展示；卡片继续展示双语姓名、身份展示文案、2–3 句双语简介、授权公开头像与可选个人主页/邮箱。既有博士后不强行归入四类，作为兼容身份排在同年四类之后。材料未收齐的简介使用克制占位文案（"个人与研究简介待补充 / Profile forthcoming"）和统一中性头像位，不显示成员详情页，不承担课题组站的成果关联或编辑功能。
...
- 成员：前台为9位成员单网格并按入学年份排序，但不公开显示年份。Studio 新增成员必须填写 `enrollmentYear` 并选择 `memberRole`（硕士生、博士生、硕转博、已毕业）；前端执行年份降序，同年按博士生、硕转博、硕士生、已毕业、其他既有身份排序，最后以本地化姓名稳定排序。`order` 仅作为隐藏旧字段保留，不再影响前台。只在收到本人确认材料后维护年份、身份、双语简介与公开照片；截至2026-09-02，9位成员均已按本人材料补齐原有资料。照片在 Sanity 保存原图，前端1:1容器只做非破坏性居中裁切；后续裁图或换图须保留公开授权并同步检查中英文页面。
...
### 2026-09-08 - People 按入学年份与成员身份排序（本地完成）

- Person Schema 新增结构化 `memberRole`，后台以单选方式提供“硕士生 / 博士生 / 硕转博 / 已毕业”四个值；新增成员必须同时填写既有必填 `enrollmentYear` 与 `memberRole`。`position` 保留为前台双语展示文案，与排序身份解耦。
- 前台排序改为：有入学年份者在前，年份从近到远；同年依次为博士生、硕转博、硕士生、已毕业、其他既有身份；同身份再按当前语言姓名稳定排序。旧 `order` 字段不再参与排序，在 Studio 中隐藏但不删除历史数据。
- 为兼容尚未回填 `memberRole` 的 Production 文档，排序纯函数会从已发布的中英文身份文案确定性识别四类；“毕业生 · 香港大学博士研究生”优先识别为已毕业，避免被其中的博士字样误排。许家奇的既有博士后身份不强行映射到四类，排在同年四类之后，且可继续编辑。
- Sanity 查询与 `PublicPerson` 类型已纳入 `memberRole`；静态回退 9 人补入已确认年份及可明确映射的身份，确保 Sanity 不可用时顺序一致。新增测试覆盖年份优先级、四类身份顺序、旧 `order` 失效、历史文案兼容、双语姓名 tie-break 与输入数组不变。
- 验证：ESLint 0 error/0 warning；Next.js 16.2.12 Production build 通过并生成24个页面；Sanity Studio build 通过；全量测试54/54通过。本轮未修改 Sanity Production、未部署 Studio/Vercel、未提交或推送 Git；Production 现有8位非博士后成员的 `memberRole` 仍待新版 Studio 部署后回填。
