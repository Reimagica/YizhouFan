...
| 当前阶段 | 2026-07-27 GitHub/Vercel 推送前本地开发已完成；等待 Sanity、对象存储与部署账号配置 |
...
- AI 问答的浏览器端只调用本站 `/api/ask`；密钥仅放服务端环境变量，问题限 800 字。当前内存限流只适合本地与结构演示，正式公开前必须改为持久化边缘限流，并配置费用告警与总额上限。
...
| `/api/ask` | DeepSeek 服务端问答、公开知识上下文、输入预算、超时、单 IP 与全站持久化限流 |
