...
test("requires an enrollment year and one of six sorting roles without rendering either on cards", async () => {
  ...
  assert.match(schemaSource, /根据入学年份和成员身份自动填入，可自行修改中文和英文内容。/);
  ...
  assert.match(positionInputSource, /if \(readOnly \|\| !generated\)/);
  assert.match(positionInputSource, /autoGenerated/);
  assert.match(positionInputSource, /readOnly=\{readOnly\}/);
